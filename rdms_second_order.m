function [table_Rho, table_H, table_P, all_subject_rhos, lme] = rdms_second_order(rows, cols, control_col_idxs, do_LME, lme_row_idxs, lme_col_idxs)
...
% control_col_idxs = indices of RDMs in cols (e.g. models) to control for
...
for row_idx = 1:numel(rows)
    ...
    for col_idx = 1:numel(cols)
        ...
        if do_LME && ismember(row_idx, lme_neural_idxs) && ismember(col_idx, lme_model_idxs)
            lme_neural = [];
            lme_model = [];
            lme_ids = [];
            lme_controls = [];
        ...
        for subj = 1:metadata.N
            ...
            for i = 1:numel(control_col_idxs) 
                control_RDMs(:,:,i) = Model(control_col_idxs(i)).RDMs(:,:,subj);
            end
