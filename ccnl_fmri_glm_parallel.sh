...
do
        ...
        do
            ...
            # send the job to NCF
            #
            sbatch_output=`sbatch -p ncf --mem 50000 -t 20-18:20 -o ${outfileprefix}_%j.out --mail-type=END --wrap="matlab -nodisplay -nosplash -nojvm -r $'ccnl_fmri_glm(context_expt(), $model, [$i]);exit'"`
            # for local testing
